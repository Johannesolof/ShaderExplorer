...
namespace DebugHelper
{
	...
	void StartupGLDiagnostics()
	{
		...
		if (!GLEW_VERSION_3_0)
		{
			FatalError("OpenGL 3.0 not supported.\n"
...
#	undef CALLBACK_
	...
	void SetupGLDebugMessages()
	{
		...
		*/
		glDebugMessageCallback(reinterpret_cast<GLDEBUGPROC>(handle_debug_message_), nullptr);
	...
	// Error reporting
	void FatalError(std::string errorString, std::string title)
	{
		if (title.empty()) {
	...
	void NonFatalError(std::string errorString, std::string title)
	{
		if (title.empty()) {
			title = "GL-Tutorial - Error";
		}
