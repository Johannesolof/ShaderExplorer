...
Texture Model::LoadTexture(const std::string & _filename, int _components) {
	...
	texture.valid = true;
	int components;
	texture.data = stbi_load(filename.c_str(), &texture.width, &texture.height, &components, _components);
...
Model::~Model()
{
	...
	glDeleteBuffers(1, &positions_bo);
	glDeleteBuffers(1, &normals_bo);
	glDeleteBuffers(1, &texture_coordinates_bo);
}
...
void Model::LoadModelFromOBJ(std::string path)
{
	...
	this->positions.resize(number_of_vertices);
	this->normals.resize(number_of_vertices);
	this->texture_coordinates.resize(number_of_vertices);
	...
	for (const auto & shape : shapes)
	{
			...
			for (int i = current_material_starting_face; i < number_of_faces; i++)
			{
				if (shape.mesh.material_ids[i] != current_material_index) {
					...
					for (int j = 0; j < 3; j++) {
						...
						this->positions[vertices_so_far + j] = glm::vec3(
						...
						if (shape.mesh.indices[i * 3 + j].normal_index == -1) {
							...
							this->normals[vertices_so_far + j] = glm::vec3(auto_normals[v]);
						}
						else {
							this->normals[vertices_so_far + j] = glm::vec3(
						...
						if (shape.mesh.indices[i * 3 + j].texcoord_index == -1) {
							...
							this->texture_coordinates[vertices_so_far + j] = glm::vec2(0.0f);
						}
						else {
							this->texture_coordinates[vertices_so_far + j] = glm::vec2(
		...
		if (number_of_materials_in_shape == 1) {
			this->meshes.back().name = shape.name;
		}
	}
	...
	glBufferData(GL_ARRAY_BUFFER, this->positions.size() * sizeof(glm::vec3),
		&this->positions[0].x, GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, this->normals.size() * sizeof(glm::vec3),
		&this->normals[0].x, GL_STATIC_DRAW);
	glVertexAttribPointer(1, 3, GL_FLOAT, false, 0, nullptr);
	glEnableVertexAttribArray(1);
	glGenBuffers(1, &this->texture_coordinates_bo);
	glBindBuffer(GL_ARRAY_BUFFER, this->texture_coordinates_bo);
	glBufferData(GL_ARRAY_BUFFER, this->texture_coordinates.size() * sizeof(glm::vec2),
		&this->texture_coordinates[0].x, GL_STATIC_DRAW);
...
void Model::Render(const bool submitMaterials)
{
	...
	{
		if (submitMaterials) {
			const Material & material = materials[mesh.material_idx];
			...
			glUniform3fv(glGetUniformLocation(current_program, "material_diffuse_color"), 1, &material.color.x); //FIXME: Compatibility with old shading model of lab3.
			glUniform3fv(glGetUniformLocation(current_program, "material_emissive_color"), 1, &material.color.x); //FIXME: Compatibility with old shading model of lab3.
